      ...
      >
        <div className="flex flex-wrap gap-1.5 max-w-[90%] overflow-hidden truncate">
          {selectedNames.length === 0 ? (
            <span className="text-slate-200 font-bold font-bold">
              {placeholder}
            </span>
          ) : (
            selectedNames.map((name, i) => (
              <span
                key={i}
                className={`text-[10px] px-2.5 py-1 rounded-lg bg-${themeColor}-50 text-${themeColor}-700 border border-${themeColor}-100 font-black`}
              >
                {name}
              </span>
            ))
        ...
        <ChevronRight
          className={`w-4 h-4 text-slate-200 font-bold transform transition-transform ${isOpen ? "rotate-90" : "-rotate-90"}`}
        />
      </div>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-1 custom-scrollbar">
              {filteredItems.length === 0 ? (
                <div className="py-6 text-center text-slate-200 font-bold font-bold text-xs">
                  ...
                  return (
                    ...
                    >
                      <div className="min-w-0 pr-1 text-right">
                        ...
                        {item.info && (
                          <p className="text-[11px] font-bold text-slate-200 font-bold">
                            {item.info}
                          </p>
                        )}
                      </div>
...
export default function EmployeePortal({
  ...
  const [selectedConfigEmployee, setSelectedConfigEmployee] =
  ...
  useEffect(() => {
    ...
    const fetchEmployees = async () => {
      const { data, error } = await supabase
        .from("employees")
        .select(
          "id, name, nationality, national_id, phone, job_title, manager, qualification, start_date, email, branch, notes, avatar_url, employee_code, role, department, salary, created_at",
        );
      if (data) {
    ...
    const channel = supabase
      ...
      .subscribe((status, error) => {
        if (error) {
          console.warn(
            "[Supabase Realtime] Subscribe error for portal employees:",
            error,
          );
        }
      });
  ...
  const handleSaveConfig = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const employeeCodeInput = form.querySelector(
      '[name="employeeCode"]',
    ) as HTMLInputElement;
    const jobTitleInput = form.querySelector(
      '[name="jobTitle"]',
    ) as HTMLInputElement;
    ...
    const najizApiKeyInput = form.querySelector(
      '[name="najizApiKey"]',
    ) as HTMLInputElement;
    ...
    const customLoginToken =
      selectedConfigEmployee.customLoginToken ||
      btoa(
        `${selectedConfigEmployee.id}-${Math.random().toString(36).substring(2, 10)}`,
      );
    ...
    try {
      console.log(
        "Attempting to save employee config:",
      ...
      try {
        ...
        const idx = backup.findIndex(
          (e: any) => e.id === selectedConfigEmployee.id,
        );
        const updatedBackup = {
      ...
      setEmployees((prev) =>
        prev.map((e) =>
          e.id === selectedConfigEmployee.id ? { ...e, ...updates } : e,
        ),
      );

      alert(
        `✅ تم حفظ إعدادات البوابة نهائياً\n👤 ${updates.username}\n🔑 ${updates.password}`,
      );
  ...
  const sendWhatsAppConfig = () => {
    ...
    window.open(
      `https://wa.me/${selectedConfigEmployee.phone}?text=${encodeURIComponent(text)}`,
      "_blank",
    );
  };
  ...
  const [najizSyncLog, setNajizSyncLog] = useState<
  ...
  const [selectedHRDateStr, setSelectedHRDateStr] = useState(
  ...
  useEffect(() => {
    ...
    const attChannel = supabase
      ...
      .subscribe((status, error) => {
        if (error) {
          console.warn(
            "[Supabase Realtime] Subscribe error for attendance:",
            error,
          );
        }
      });
    ...
    const leaveChannel = supabase
      ...
      .subscribe((status, error) => {
        if (error) {
          console.warn(
            "[Supabase Realtime] Subscribe error for leave requests:",
            error,
          );
        }
      });
    ...
    if (method === "location") {
      ...
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          console.error(error);
          alert(
            "تعذر الوصول إلى الموقع الجغرافي. يرجى تفعيل الصلاحيات والمحاولة مجدداً.",
          );
          setGeoLocating(false);
          setShowCheckInModal(false);
        },
  ...
  const handleApplyLeave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      alert(
        "✅ تم إرسال طلب الإجازة للمدير المباشر بنجاح. سيصلك إشعار فور اتخاذ قرار.",
      );
  ...
  const handleRunNajizSync = async () => {
    ...
    if (!activeKey) {
      alert(
        "الرجاء تعبئة مفتاح API الخاص بناجز في المحاكي أولاً لبدء سحب البيانات.",
      );
      return;
    }
    ...
    setNajizSyncLog([
      {
        msg: "بدء الاتصال الآمن مع بوابة ناجز لوزارة العدل...",
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      setNajizSyncLog((prev) => [
        ...prev,
        { msg: steps[i], time: new Date().toLocaleTimeString() },
      ]);
    }
    ...
    try {
      ...
      const sqlSimCase = {
        ...
        summary:
          "طلب إلزام بسداد المستحقات المالية عن الدفعة الختامية المبرمة طبقاً للنظام التجاري الجديد.",
        details:
          "تم جمع البيانات ومزامنتها بنجاح عن طريق أداة السحب من بوابة ناجز للموظف.",
        attachments_count: 1,
        priority: "high",
        metadata: JSON.stringify({
          clientName: clientNameSim,
          lastSessionDate: new Date().toISOString().split("T")[0],
          nextSessionDate: new Date(Date.now() + 86400000 * 14)
            .toISOString()
            .split("T")[0],
      ...
      const { data: caseDoc, error: caseErr } = await supabase
        .from("cases")
        .insert(sqlSimCase)
        .select()
        .single();
      ...
      if (loggedInEmployee && caseDoc) {
        const updatedCases = [
          ...(loggedInEmployee.assignedCases || []),
          caseDoc.id,
        ];
      ...
      const testTask: Partial<Task> = {
        ...
        description:
          "مراجعة الملحقات المستلمة من ناجز وكتابة التقرير الأولي للمدير العام.",
        status: "todo",
        priority: "high",
      ...
      await writeAuditLog(
        "مزامنة ناجز",
        `تم سحب الدعوى التجارية رقم ${simCaseNumber} وتعيينها للموظف بنجاح`,
        loggedInEmployee!,
      );
      ...
      setNajizSyncLog((prev) => [
        ...prev,
        {
          msg: `حدث خطأ أثناء الاتصال بالـ API: ${e.message}`,
          time: new Date().toLocaleTimeString(),
        },
      ]);
    } finally {
  ...
  const handleToggleTaskStatus = async (taskId: string, newStatus: string) => {
    try {
      ...
      if (loggedInEmployee) {
        await writeAuditLog(
          "تحديث حالة المهمة",
          `تحديث حالة المهمة إلى: ${newStatus}`,
          loggedInEmployee,
        );
      }
    } catch (err) {
      ...
      const response = await fetch("/api/ai/chat", {
        ...
        body: JSON.stringify({
          message: `أنت مساعد قانوني مهني خبير وخاص بالموظفين في منصة مكتب العدالة. أجب عن الطلب التالي بكل احترافية قانونية ودقة مع صياغة تليق بالقضاء السعودي:\n\nالطلب: ${aiPrompt}\n\nنوع الخدمة المطلوبة: ${
            aiTab === "consult"
              ? "استشارة وتحليل أنظمة"
              : aiTab === "memo"
                ? "صياغة مذكرة دفاع وجواب"
                : "تلخيص ملفات قانونية وصياغة بنود"
          }`,
        }),
      });
      const data = await response.json();
      setAiResponse(
        data.reply || data.text || "لم يتم استلام رد من النظام الذكي.",
      );
    } catch {
      setAiResponse(
        "حدث خطأ في الاتصال بنظام الذكاء الاصطناعي. الرجاء التأكد من تشغيل الخادم.",
      );
    } finally {
  ...
  if (isAdmin) {
    ...
    return (
      <div
        className="flex-1 h-full flex flex-col bg-slate-50 overflow-y-auto p-10 font-sans"
        dir="rtl"
      >
          ...
          <header className="bg-white border border-slate-200 rounded-[2.5rem] p-10 shadow-[0_10px_40px_-15px_rgba(30,58,138,0.06)] relative overflow-hidden group">
            ...
            <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-10">
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <h1 className="text-3xl font-black text-slate-900 leading-tight">
                      إدارة تفويض وصلاحيات الكادر المهني
                    </h1>
                    <p className="text-slate-700 font-bold text-lg">
                      تحكم كامل في بوابات الوصول المخصصة وتعيين المهام المركزية
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="relative w-full md:w-80">
                <label className="text-[10px] font-black tracking-widest text-blue-600 block mb-2 pr-1">
                  تحديد الموظف المراد تهيئته:
                </label>
                <select
                ...
                {employees.length > 0 ? (
                        ...
                        >
                            ...
                            <div>
                              <h4 className="font-black text-slate-900 text-lg mb-1 flex items-center flex-wrap gap-2">
                                ...
                                {hasAccount ? (
                                  ...
                                  <span className="text-[11px] font-black bg-amber-50 text-amber-400 font-black px-2 py-0.5 rounded-full border border-amber-200">
                                    بانتظار التهيئة ⚠️
                                  </span>
                                )}
                              </h4>
                              <p className="text-[10px] text-slate-200 font-bold font-bold">
                                {e.jobTitle || "محامي / مستشار بالتمرين"}
                              </p>
                            </div>
                            <div
                              className={`w-10 h-10 rounded-full flex items-center justify-center border ${hasAccount ? "bg-emerald-50 border-emerald-100 text-emerald-600" : "bg-amber-50 border-amber-100 text-amber-400 font-black"}`}
                            >
                          ...
                          <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 space-y-2 mt-auto">
                            <div className="flex justify-between text-[10px] font-bold">
                              ...
                              {hasAccount ? (
                                <span
                                  className="text-blue-600 font-mono text-left"
                                  dir="ltr"
                                >
                                  {e.username}
                                </span>
                              ) : (
                                <span className="text-amber-400 font-black italic">
                                  مؤقت: emp_{e.id.toLowerCase()}
                                </span>
                              )}
                            </div>
                            <div className="flex justify-between text-[10px] font-bold">
                              <span className="text-slate-200 font-bold">
                                أقسام البوابة الفعّالة:
                              </span>
                              <span className="text-emerald-600">
                                {(e.sidebarConfig && e.sidebarConfig.length) ||
                                  0}{" "}
                                أقسام مفوضة
                              </span>
                            </div>
                          ...
                          >
                            {hasAccount
                              ? "تعديل الصلاحيات والمزايا ⚖️"
                              : "تهيئة البوابة وإصدار الصلاحيات 🔑"}
                          </button>
                        </div>
                      );
                    })}
                  ...
                  <div className="py-32 bg-white border-2 border-dashed border-slate-200 rounded-[3rem] flex flex-col items-center justify-center text-center gap-8 shadow-sm">
                    ...
                    <div>
                      ...
                      <p className="text-slate-700 font-bold max-w-sm mt-2 mx-auto">
                        عند إضافة كادر وظيفي جديد من شاشة "شؤون الموظفين"، سيتم
                        عرضهم هنا تلقائياً لتهيئتهم وضبط بواباتهم الخاصة.
                      </p>
                    </div>
                  </div>
                )}
                    ...
                    <div className="bg-white border border-slate-200 rounded-[3rem] p-10 shadow-[0_20px_50px_-15px_rgba(30,58,138,0.06)] space-y-8 relative overflow-hidden group hover:border-blue-200 transition-all duration-500">
                      ...
                      <div className="flex items-center gap-4 border-b border-slate-100 pb-6">
                        ...
                        <div>
                          <h3 className="text-lg font-black text-slate-900">
                            إعداد المصادقة والدخول
                          </h3>
                          <p className="text-[10px] font-bold text-slate-200 font-bold uppercase tracking-widest mt-0.5">
                      ...
                      <div className="space-y-6">
                        <div className="space-y-2">
                          <label className="text-[11px] font-black text-slate-700 block pr-2">
                            اسم المستخدم الموحد
                          </label>
                          <input
                            name="username"
                    ...
                    <div className="bg-white border border-slate-200 rounded-[3rem] p-10 shadow-sm space-y-6">
                      ...
                      <div className="space-y-4">
                        <p className="text-[11px] font-bold text-slate-700 leading-relaxed text-right">
                          يتيح هذا الرمز سحب كافة التحديثات المباشرة من بوابة
                          ناجز (القضايا، المواعيد، القرارات) وإسقاطها فوراً في
                          ملف الموظف.
                        </p>
                        <input
                          name="najizApiKey"
                  ...
                  <div className="lg:col-span-8 space-y-8">
                    <div className="bg-white border border-slate-200 rounded-[3rem] p-10 shadow-sm space-y-8">
                      <div className="flex items-center gap-4 border-b border-slate-100 pb-6">
                        ...
                        <div className="text-right">
                          <h3 className="text-lg font-black text-slate-900">
                            تفويض الصلاحيات والوصول للبيانات
                          </h3>
                          <p className="text-[10px] font-bold text-slate-200 font-bold uppercase tracking-widest mt-0.5">
                            Permissions & Data Delegation
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <CustomMultiSelectDropdown
                          ...
                          themeColor="sky"
                          selectedIds={
                            selectedConfigEmployee.sidebarConfig || []
                          }
                          items={[
                            {
                              id: "dashboard",
                              name: "لوحة القيادة الرئيسية والملخص",
                            },
                            {
                              id: "cases",
                              name: "بوابة القضايا وعقود الترافع",
                            },
                            { id: "tasks", name: "المهام وتوزيع الأعمال" },
                            {
                              id: "documents",
                              name: "أرشيف وحقيبة المستندات والملفات",
                            },
                            {
                              id: "ai",
                              name: "مساعد ومستشار الذكاء الاصطناعي (AI)",
                            },
                            {
                              id: "ai-search",
                              name: "مرصد الأنظمة والبحث الذكي",
                            },
                            { id: "clients", name: "سجل العملاء والموكلون" },
                            {
                              id: "najiz",
                              name: "مركز مكاملة وسحب بيانات ناجز",
                            },
                          ]}
                          onChange={(ids) => {
                            const updatedEmp = {
                        ...
                        <CustomMultiSelectDropdown
                          ...
                          items={[
                            {
                              id: "view_all",
                              name: "الاطلاع على كافة بيانات المكتب (مدير عام)",
                            },
                            {
                              id: "edit_all",
                              name: "تعديل وحذف كافة ملفات النظام",
                            },
                            {
                              id: "add_cases",
                              name: "إمكانية إدخال وإضافة قضايا جديدة",
                            },
                            {
                              id: "edit_cases",
                              name: "تعديل وتحديث لائحة القضايا المسندة",
                            },
                            {
                              id: "add_clients",
                              name: "إضافة عملاء جدد للمكتب",
                            },
                            {
                              id: "edit_clients",
                              name: "تعديل وحذف بيانات العملاء",
                            },
                            {
                              id: "task_management",
                              name: "إسناد وتغيير حالة مهام زملائه",
                            },
                            {
                              id: "access_financials",
                              name: "الاطلاع على التقارير المالية والفواتير",
                            },
                            {
                              id: "najiz_sync",
                              name: "تفعيل المزامنة المباشرة مع ناجز",
                            },
                          ]}
                          onChange={(ids) => {
                            const updatedEmp = {
                            ...
                            {(availableCases.length > 0
                              ...
                              >
                                <input
                                  type="checkbox"
                                  checked={(
                                    selectedConfigEmployee.assignedCases || []
                                  ).includes(c.id)}
                                  onChange={(e) => {
                                    const ids =
                                      selectedConfigEmployee.assignedCases ||
                                    ...
                                    const updatedEmp = {
                                      ...selectedConfigEmployee,
                                      assignedCases: e.target.checked
                                        ? [...ids, c.id]
                            ...
                            {(availableClients.length > 0
                              ...
                              >
                                <input
                                  type="checkbox"
                                  checked={(
                                    selectedConfigEmployee.assignedClients || []
                                  ).includes(cl.id)}
                                  onChange={(e) => {
                                    const ids =
                                      selectedConfigEmployee.assignedClients ||
                                ...
                                <span className="text-slate-800 text-xs font-bold">
                                  {cl.name}{" "}
                                  <span className="text-slate-400 mr-2 text-[10px] font-normal">
                                    {cl.phone || cl.email}
                                  </span>
                                </span>
                              </label>
  ...
  if (!loggedInEmployee) {
    ...
    return (
      <div
        className="min-h-screen flex items-center justify-center p-8 bg-[#F8FAFC] font-sans relative overflow-hidden"
        dir="rtl"
      >
        ...
        >
          <div className="bg-white border border-slate-200 rounded-[3rem] p-12 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] space-y-10 relative overflow-hidden">
            ...
            <div className="text-center space-y-4">
              ...
              <p className="text-slate-700 font-bold text-sm bg-slate-50 border border-slate-100 px-4 py-2 rounded-full inline-block mx-auto">
                مركز العمل القضائي المتكامل
              </p>
            </div>
            ...
            <form onSubmit={handleLogin} className="space-y-8">
              <div className="space-y-6">
                <div className="space-y-3">
                  <label className="text-xs font-black text-slate-700 pr-2">
                    اسم المستخدم
                  </label>
                  <div className="relative">
                    <input
              ...
              <div className="text-center pt-2">
                <p className="text-[10px] text-slate-200 font-bold font-bold leading-relaxed">
                  نظام مشفر ومحمي. يرجى التواصل مع الإدارة الفنية لمكتب العدالة
                  في حال واجهت صعوبات في الدخول.
                </p>
              </div>
            </form>
          </div>
  ...
  const updateTaskFromPortal = async (taskId: string, newStatus: string) => {
  ...
  const [selectedCalendarDateStr, setSelectedCalendarDateStr] =
    useState<string>(() => {
  ...
  const mySessions = React.useMemo(() => {
    return myCases.map((c, i) => {
      ...
      return {
        ...
        hall: c.circuitNumber
          ? `الدائرة ${c.circuitNumber}`
          : `الدائرة القضائية ${["الخامسة تجاري", "الثالثة حقوقي", "الأولى عمالي", "السادسة مروري"][i % 4]}`,
        courtName: c.courtName || "المحكمة التجارية بالرياض",
        judgeName:
          c.judgeName ||
          c.judge_name ||
          `فضيلة الشيخ ${["عبد الله بن ناصر الغامدي", "عمر بن صالح الدوسري", "بندر بن خالد العتيبي", "صالح بن علي الراجحي"][i % 4]}`,
        sessionType: [
          "تقديم الدفاع الرئيسي",
          "تبادل المذكرات والجواب",
          "طلب الخبرة الفنية الهندسي",
          "جلسة مرافعة ختامية",
        ][i % 4],
        status: (c.status === "closed" ? "completed" : "upcoming") as
          | "upcoming"
          | "completed"
          | "canceled",
  ...
  const personalKPIs = React.useMemo(() => {
    ...
    const completedCases =
      myCases.filter(
        (c) => c.status === "closed" || c.status === "judgment_issued",
      ).length || 2;
    const ongoingCases =
      totalCases - completedCases > 0 ? totalCases - completedCases : 1;

    const totalT = myTasks.length;
    const completedTasksOnTime =
      myTasks.filter((t) => t.status === "completed" || t.status === "done")
        .length || 3;
    const pendingTasks =
      totalT - completedTasksOnTime > 0 ? totalT - completedTasksOnTime : 1;
    ...
    return {
      ...
      totalSessions,
      caseCompletionRate:
        Math.round((completedCases / (totalCases || 1)) * 100) || 75,
      taskOnTimeRate:
        Math.round((completedTasksOnTime / (totalT || 1)) * 100) || 85,
    };
  ...
  const kpiChartData = [
    {
      name: "القضايا المنجزة",
      "المنجز الفعلي": personalKPIs.completedCases,
      "المستهدف الربعي": 5,
    },
    {
      name: "المهام في موعدها",
      "المنجز الفعلي": personalKPIs.completedTasksOnTime,
      "المستهدف الربعي": 8,
    },
    {
      name: "الجلسات المحضرة",
      "المنجز الفعلي": personalKPIs.preparedSessions,
      "المستهدف الربعي": 6,
    },
  ];
  ...
  const taskPieData = [
    {
      name: "منجزة في الوقت",
      value: personalKPIs.completedTasksOnTime,
      color: "#10b981",
    },
    {
      name: "تحت التنفيذ والعمل",
      value: personalKPIs.pendingTasks,
      color: "#f59e0b",
    },
  ];

  const selectedDaySessions = mySessions.filter(
    (s) => s.date === selectedCalendarDateStr,
  );

  return (
    <div
      className="flex-1 h-full flex flex-row bg-[#F8FAFC] text-slate-900 font-sans overflow-hidden"
      dir="rtl"
    >
      ...
      <aside className="w-72 border-l border-slate-200 bg-white hidden lg:flex flex-col p-6 space-y-8 overflow-y-auto shadow-sm">
        ...
        <nav className="space-y-1">
          <div className="text-[10px] font-black text-slate-200 font-bold uppercase tracking-widest px-3 mb-3 text-right">
            قائمة الوصول السريع
          </div>
          {[
            ...
            .filter((item) => {
              ...
              const isManagerial =
                userPerms.includes("view_all") ||
                userPerms.includes("edit_all");
        ...
        <header className="p-6 md:p-8 border-b border-slate-200 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white relative overflow-hidden">
          <div className="z-10 flex items-center gap-5">
            ...
            >
              <h1 className="text-2xl font-black text-white tracking-tight">
                أهلاً بك، الأستاذ {loggedInEmployee.name.split(" ")[0]} 👋
              </h1>
              <p className="text-slate-300 font-bold text-xs mt-0.5">
                مركز العمل القضائي المتكامل • بيئة سحابية آمنة
              </p>
            </div>
          </div>
          ...
          <div className="z-10 flex items-center gap-3 md:gap-4">
            ...
            <div className="relative">
              ...
              >
                <Bell
                  className={`w-5 h-5 ${urgentTasks.length > 0 ? "animate-bounce" : ""}`}
                />
                {urgentTasks.length > 0 && (
                  ...
                  <div className="flex items-center gap-5 text-right w-full">
                    ...
                    <div>
                      <h4 className="text-lg font-black text-slate-900 flex items-center gap-3">
                        ...
                        <span className="text-[10px] bg-rose-600 text-white px-2.5 py-1 rounded-full font-black animate-pulse">
                          هام جداً
                        </span>
                      </h4>
                      <p className="text-xs text-slate-700 mt-1 font-bold leading-relaxed max-w-2xl">
                        المهمة المسندة{" "}
                        <span className="text-rose-600 font-black underline underline-offset-4 decoration-rose-200">
                          "{task.title}"
                        </span>{" "}
                        على وشك انتهاء صلاحيتها. يرجى توثيق المنجز فوراً.
                      </p>
                    </div>
                  </div>
            ...
            >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 grid grid-cols-2 md:grid-cols-4 gap-6">
                  {[
                    ...
                    {
                      label: "المهام المعلقة / قيد العمل",
                      value: myTasks.filter(
                        (t) =>
                          t.status === "todo" ||
                    ...
                    <div
                      key={i}
                      className={`${stat.bg} border border-[#e2e8f0] p-6 rounded-[2.5rem] flex flex-col justify-between shadow-sm relative overflow-hidden min-h-[160px] group  transition-all`}
                    >
                      <stat.icon
                        className={`w-8 h-8 ${stat.color} mb-4 relative z-10 transition-transform `}
                      />
                  ...
                  <div className="col-span-2 md:col-span-4 bg-white border border-slate-200 p-8 rounded-[2.5rem] flex items-center justify-between shadow-sm hover:border-slate-300 transition-colors">
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-slate-200 font-bold font-bold">
                        بناءً على المهام المنجزة مقارنة بالمهام الكلية المسندة
                        لك.
                      </p>
                    </div>
                    <div className="flex items-center gap-4 w-1/2">
                      <div className="w-full h-3 bg-slate-100 rounded-full overflow-hidden flex-1 shadow-inner">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{
                            width: `${myTasks.length > 0 ? Math.round((myTasks.filter((t) => t.status === "completed" || t.status === "done").length / myTasks.length) * 100) : 0}%`,
                          }}
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 pb-6 border-b border-slate-100">
                      <div className="space-y-1 text-right">
                        ...
                        <p className="text-xs text-slate-200 font-bold font-bold">
                          باقة ذكية تدمج جلسات المحاكم المسندة إليك مع الإجازات
                          (المعتمدة والمعلقة) والمهام
                        </p>
                      </div>
                    ...
                    {(() => {
                      ...
                      for (let i = 0; i < firstDayIndex; i++) {
                        cells.push(
                          <div
                            key={`empty-${i}`}
                            className="min-h-[110px] bg-slate-50/20 border border-slate-100 rounded-2xl"
                          />,
                        );
                      }
                      ...
                      for (let day = 1; day <= daysInMonth; day++) {
                        ...
                        const dayHearings = (hearings || []).filter((h) => {
                          const isCaseAssigned =
                            loggedInEmployee?.assignedCases?.includes(
                              h.caseId || h.caseNumber,
                            );
                        ...
                        const dayLeaves = (leaveRequests || []).filter((r) => {
                          return (
                            r.employeeId === loggedInEmployee?.id &&
                            dateStr >= r.startDate &&
                            dateStr <= r.endDate
                          );
                        });
                        ...
                        cells.push(
                          ...
                          >
                            <div className="flex items-center justify-between">
                              <span
                                className={`text-[11px] font-black w-6 h-6 rounded-full flex items-center justify-center ${isTodayStr ? "bg-indigo-600 text-white font-black" : "text-slate-700"}`}
                              >
                                {day}
                              </span>
                              ...
                              <div className="flex items-center gap-1">
                                {dayHearings.length > 0 && (
                                  <span
                                    className="w-1.5 h-1.5 bg-rose-500 rounded-full animate-pulse"
                                    title="جلسات قضائية"
                                  />
                                )}
                                {dayLeaves.some(
                            ...
                            <div className="mt-2 space-y-1 overflow-hidden">
                              {dayHearings.map((h, hIdx) => (
                                <div
                                  key={hIdx}
                                  className="bg-rose-50 border border-rose-100 text-rose-700 p-1 rounded-lg text-[11px] font-extrabold truncate"
                                  title={h.caseName}
                                >
                                  ⚖️ {h.time} {h.caseName}
                                </div>
                              ...
                              {dayLeaves.map((l, lIdx) => (
                                <div
                                  key={lIdx}
                                  className={`p-1 rounded-lg text-[11px] font-extrabold truncate ${l.status === "approved" ? "bg-emerald-50 border border-emerald-100 text-emerald-700" : "bg-amber-50 border border-amber-100 text-amber-400 font-black"}`}
                                >
                                  🌴{" "}
                                  {l.status === "approved"
                                    ? "إجازة معتمدة"
                              ...
                              {dayTasks.map((t, tIdx) => (
                                <div
                                  key={tIdx}
                                  className="bg-sky-50 border border-sky-100 text-sky-700 p-1 rounded-lg text-[11px] font-extrabold truncate"
                                  title={t.title}
                                >
                                  📋 {t.title}
                                </div>
                            ...
                            {selectedCalendarDate === dateStr && (
                              <div
                                className="absolute top-[105%] right-0 left-0 bg-slate-900 text-white border border-slate-800 p-4 rounded-2xl shadow-2xl z-40 space-y-3 cursor-default"
                                onClick={(e) => e.stopPropagation()}
                              >
                                <div className="flex items-center justify-between border-b border-white/10 pb-2">
                                  <span className="text-xs font-black text-white font-bold">
                                    أجندة يوم {day} ({dateStr})
                                  </span>
                                  <button
                                    onClick={() =>
                                ...
                                {dayHearings.length > 0 && (
                                  <div className="space-y-1.5">
                                    <p className="text-[10px] font-black text-rose-400 flex items-center gap-1">
                                      ⚖️ جلسات المرافعة ومواعيد الدوائر:
                                    </p>
                                    {dayHearings.map((h, hIdx) => (
                                      <div
                                        key={hIdx}
                                      ...
                                      >
                                        ...
                                        <div className="text-slate-200 font-bold text-[11px] mt-1 text-right">
                                          الساعة {h.time} | الدائرة{" "}
                                          {h.court || "غير محدد"} | قضية رقم:{" "}
                                          {h.caseNumber}
                                        </div>
                                      </div>
                                    ))}
                                  ...
                                  <div className="space-y-1.5 border-t border-white/5 pt-2">
                                    ...
                                    {dayLeaves.map((l, lIdx) => (
                                      ...
                                      >
                                        ...
                                        <span
                                          className={`px-2 py-0.5 rounded text-[10px] font-black ${l.status === "approved" ? "bg-emerald-500/20 text-emerald-400" : "bg-amber-500/20 text-amber-300"}`}
                                        >
                                          {l.status === "approved"
                                            ? "مقبولة ومعتمدة"
                                            : "طلب معلق"}
                                ...
                                {dayTasks.length > 0 && (
                                  <div className="space-y-1.5 border-t border-white/5 pt-2">
                                    <p className="text-[10px] font-black text-sky-400 flex items-center gap-1">
                                      📋 تسليمات المهام اليومية:
                                    </p>
                                    {dayTasks.map((t, tIdx) => (
                                      <div
                                        key={tIdx}
                        ...
                        <div>
                          <div className="grid grid-cols-7 gap-3 mb-3 text-center">
                            {weekdayNames.map((name) => (
                              <div
                                key={name}
                                className="text-[10px] font-black text-slate-200 font-bold bg-slate-50 py-2.5 rounded-xl border border-slate-100"
                              >
                                {name}
                              </div>
                            ))}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="bg-white border border-slate-200 p-8 rounded-[3rem] hover:border-purple-300 transition-all group relative overflow-hidden shadow-sm">
                  ...
                  <div className="flex items-center justify-between mb-6">
                    <span className="text-[10px] font-black text-slate-200 font-bold uppercase tracking-widest">
                      كفاءة الأداء التشغيلي
                    </span>
                    <Activity className="w-5 h-5 text-purple-600" />
                  </div>
                  <div className="flex items-baseline gap-2">
                    <h4 className="text-4xl font-black text-slate-900">
                      {personalKPIs.caseCompletionRate}%
                    </h4>
                    <span className="text-[10px] text-slate-700 font-bold">
                      تقدير ممتاز
                    </span>
                ...
                <div className="xl:col-span-8 bg-white border border-slate-200 rounded-[3.5rem] p-10 space-y-10 relative overflow-hidden shadow-sm">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-[11px] text-slate-700 font-bold mt-1.5 uppercase tracking-wider">
                    ...
                    <div className="flex items-center gap-3 bg-slate-50 p-2 rounded-2xl border border-slate-200">
                      ...
                      <span className="text-[12px] font-black text-blue-600 min-w-[120px] text-center">
                        {calendarDate.toLocaleString("ar-EG", {
                          month: "long",
                          year: "numeric",
                        })}
                      </span>
                  ...
                  <div className="grid grid-cols-7 gap-4">
                    {DAYS_OF_WEEK_AR.map((day) => (
                      <span
                        key={day}
                        className="text-[10px] text-slate-200 font-bold font-black text-center uppercase tracking-tighter"
                      >
                        {day}
                      </span>
                    ))}
                          ...
                          className={`aspect-square rounded-[1.5rem] text-sm font-black flex flex-col items-center justify-center relative transition-all group ${
                            isSelected
                              ...
                              : hasSess
                                ...
                                : "bg-slate-50 text-slate-200 font-bold hover:bg-slate-100 hover:text-slate-200 font-bold border border-transparent"
                          }`}
                        >
                          <span>{dayNum}</span>
                          {hasSess && !isSelected && (
                            <span className="w-1.5 h-1.5 bg-amber-500 rounded-full absolute bottom-2.5 animate-pulse" />
                          )}
                        </button>
                      );
                    })}
                  ...
                  {selectedDaySessions.length === 0 ? (
                    ...
                    <div className="space-y-4 flex-1 overflow-y-auto custom-scrollbar">
                      {selectedDaySessions.map((s) => (
                        <div
                          key={s.id}
                          className="p-6 bg-slate-50 shadow-sm rounded-[2.5rem] space-y-4 border border-slate-100 group/item transition-all hover:border-blue-200"
                        >
                          <div className="flex items-center justify-between">
                            <span className="bg-blue-100 text-blue-700 border border-blue-200 px-3 py-1 rounded-xl text-[11px] font-black">
                              {s.sessionType}
                            </span>
                            <span className="text-[11px] text-slate-200 font-bold font-bold">
                              {s.time}
                            </span>
                          </div>
                          ...
                          <div className="flex items-center gap-3 pt-4 border-t border-slate-200">
                            ...
                            <div className="text-right">
                              <p className="text-[10px] text-slate-200 font-bold font-black">
                                الموقع
                              </p>
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="w-full py-4 bg-blue-600 hover:bg-blue-500 text-white rounded-2xl font-black text-xs flex items-center justify-center gap-2 shadow-xl shadow-blue-600/20 transition-all"
                          >
                            دخول ناجز 🔗
                          </a>
                        </div>
                  ...
                  <div className="relative z-10 space-y-8">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[11px] text-slate-700 font-bold mt-1.5 uppercase tracking-wider">
                          نظرة عامة على الرصيد المستحق والمستهلك
                        </p>
                      </div>
                      <button
                    ...
                    <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 flex flex-col gap-6">
                      <div className="flex justify-between items-end">
                        <span className="text-4xl font-black text-amber-500">
                          ...
                          <span className="text-sm text-slate-700">يوماً</span>
                        </span>
                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-200 font-bold">
                          إجمالي الرصيد السنوي: {vacationBalance.earned}
                        </span>
                      </div>
                      ...
                      <div className="relative w-full h-4 bg-slate-200 rounded-full overflow-hidden">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{
                            width: `${vacationBalance.earned === 0 ? 0 : (vacationBalance.remaining / vacationBalance.earned) * 100}%`,
                          }}
                ...
                <div className="bg-white border border-slate-200 rounded-[3.5rem] p-8 shadow-sm">
                    ...
                    <div className="flex gap-2">
                      <button
                        onClick={() =>
                          setHrCalendarDate(
                            new Date(
                              hrCalendarDate.getFullYear(),
                              hrCalendarDate.getMonth() - 1,
                              1,
                            ),
                          )
                        }
                      ...
                      <span className="text-xs font-black min-w-[100px] text-center pt-2">
                        {hrCalendarDate.toLocaleString("ar-EG", {
                          month: "short",
                          year: "numeric",
                        })}
                      </span>
                  ...
                  <div className="grid grid-cols-7 gap-3 text-center">
                    {DAYS_OF_WEEK_AR.map((d) => (
                      <span
                        key={d}
                        className="text-[11px] font-black text-slate-200 font-bold p-2"
                      >
                        {d}
                      </span>
                    ))}
                    {hrCalendarData.map((dNum, i) => {
                      ...
                      const isPending = dayLeaves.some(
                        (r) => r.status === "pending",
                      );
                      const isApproved = dayLeaves.some(
                        (r) => r.status === "approved",
                      );
                      return (
                        <div
                          ...
                          title={
                            dayLeaves.length > 0
                              ? `حالة الطلب: ${isApproved ? "معتمد" : "معلق"}`
                              : undefined
                          }
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                ...
                <div className="bg-white border border-slate-200 rounded-[3.5rem] p-10 space-y-8 shadow-sm">
                  ...
                  <div className="space-y-4 max-h-[400px] overflow-y-auto custom-scrollbar pr-2">
                    {myCases.length === 0 ? (
                      <div className="py-20 text-center text-slate-200 font-bold text-xs font-bold">
                        لا يوجد قضايا نشطة مسندة حالياً.
                      </div>
                    ) : (
                      myCases.slice(0, 5).map((c) => (
                        ...
                        >
                          <div className="flex items-center gap-5 text-right w-full">
                            <div
                              className={`w-12 h-12 rounded-xl flex items-center justify-center font-black transition-colors shrink-0 ${c.priority === "high" ? "bg-rose-50 text-rose-600 border border-rose-100" : "bg-blue-50 text-blue-600 border border-blue-100"}`}
                            >
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <div className="flex items-center gap-3 mt-1.5 overflow-hidden">
                                <span className="text-[10px] font-black text-slate-200 font-bold font-mono tracking-tighter shrink-0">
                                  {c.caseNumber}
                                </span>
                                <span className="w-1 h-1 bg-slate-200 rounded-full shrink-0" />
                                <span className="text-[10px] font-black text-blue-600 truncate">
                                  {c.courtName}
                                </span>
                              </div>
                            </div>
                ...
                <div className="bg-white border border-slate-200 rounded-[3.5rem] p-10 space-y-8 shadow-sm">
                  <div className="flex items-center justify-between border-b border-slate-100 pb-6">
                    ...
                    <button
                      ...
                      className="text-[10px] font-black text-amber-400 font-black hover:underline"
                    >
                      إدارة المهام
                    </button>
                  </div>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex-1 min-w-0">
                              <h5 className="text-sm font-black text-slate-900 flex items-center gap-3">
                                <span
                                  className={`w-2 h-2 rounded-full shrink-0 ${t.priority === "high" ? "bg-rose-600 shadow-[0_0_8px_rgba(225,29,72,0.3)]" : "bg-slate-300"}`}
                                />
                            ...
                            <span
                              className={`text-[11px] font-black px-3 py-1 rounded-full shrink-0 ${t.status === "completed" || t.status === "done" ? "bg-emerald-100 text-emerald-700 border border-emerald-200" : "bg-amber-100 text-amber-400 font-black border border-amber-200"}`}
                            >
                              {t.status === "completed" || t.status === "done"
                                ? "موثقة ومنجزة ✅"
                                : "جارية للمتابعة ⏳"}
              ...
              <div className="flex flex-col md:flex-row justify-between items-center bg-white p-8 rounded-[2.5rem] border border-blue-100 shadow-sm gap-6">
                <div>
                  ...
                  <p className="text-xs text-blue-900 font-bold mt-1">
                    تتبع الحضور، تقديم طلبات الإجازة، والمستندات الشخصية.
                  </p>
                </div>
                <div className="flex gap-3">
                  <div className="hidden sm:flex bg-blue-50 border border-blue-100 px-6 py-3 rounded-2xl items-center gap-4">
                    <div className="text-right">
                      <span className="text-[11px] text-blue-400 font-black block uppercase">
                ...
                <div className="flex flex-col md:flex-row items-center gap-10">
                  ...
                  <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-4 w-full">
                    {[
                      ...
                      <div
                        key={i}
                        className={`${stat.bg} p-6 rounded-3xl border border-white/50 text-center flex flex-col items-center justify-center`}
                      >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="lg:col-span-4 bg-white border border-blue-100 rounded-[2.5rem] p-8 shadow-sm flex flex-col">
                  ...
                  <div className="flex-1 space-y-4">
                    {attendance.length === 0 ? (
                      <div className="py-20 text-center text-slate-200 font-bold text-xs font-bold">
                        لا يوجد سجلات حضور مسجلة لهذا الشهر.
                      </div>
                    ) : (
                      attendance.slice(0, 5).map((record) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-10 h-10 rounded-xl flex items-center justify-center text-white ${record.status === "late" ? "bg-amber-500" : "bg-emerald-500"}`}
                            >
                              {record.status === "late" ? (
                ...
                <div className="lg:col-span-4 bg-white border border-blue-100 rounded-[2.5rem] p-8 shadow-sm flex flex-col">
                  <div className="flex items-center justify-between mb-6 border-b border-blue-50 pb-4">
                    ...
                    <div className="flex gap-1">
                      <button
                        ...
                        className="p-1 hover:bg-slate-100 rounded-lg text-slate-200 font-bold"
                      >
                        <ChevronRight className="w-4 h-4" />
                      </button>
                      <button
                  ...
                  <div className="grid grid-cols-7 gap-1 mb-4 text-center">
                    {DAYS_OF_WEEK_AR.map((d) => (
                      <span
                        key={d}
                        className="text-[11px] font-black text-slate-200 font-bold uppercase"
                      >
                        {d}
                      </span>
                    ))}
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {hrCalendarData.map((day, i) => {
                      ...
                      const isPending = leaveRequests.some(
                        (r) =>
                          r.startDate <= dateStr &&
                          r.endDate >= dateStr &&
                          r.status === "pending",
                ...
                <div className="lg:col-span-4 bg-white border border-blue-100 rounded-[2.5rem] p-8 shadow-sm flex flex-col h-full">
                  ...
                  <div className="space-y-4 h-[400px] overflow-y-auto custom-scrollbar pr-1">
                    {leaveRequests.length === 0 ? (
                      <div className="py-20 text-center text-slate-200 font-bold text-xs font-bold">
                        لم تتقدم بأی طلبات إجازة حتى الآن.
                      </div>
                    ) : (
                        ...
                        >
                          <div className="flex justify-between items-center">
                            <h4 className="text-xs font-black text-slate-900">
                              إجازة{" "}
                              {req.type === "vacation" ? "اعتيادية" : "مرضية"}
                            </h4>
                            <span
                              className={`text-[10px] px-2 py-1 rounded-full font-black ${
                                req.status === "approved"
                                  ? "bg-emerald-50 text-emerald-600 border border-emerald-100"
                                  : "bg-amber-50 text-amber-400 font-black border border-amber-100"
                              }`}
                            >
              ...
              <div className="bg-white p-10 rounded-[2.5rem] border border-blue-100 shadow-sm relative overflow-hidden text-slate-900">
                ...
                <div className="flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-slate-700 font-bold mt-2 text-sm">
                      تحليلات الأداء الشهري بناءً على المهام، الدقة، والالتزام
                      بالمواعيد.
                    </p>
                  </div>
                ...
                <div className="bg-white border-2 border-cyan-100 rounded-[2.5rem] p-10 space-y-6 shadow-sm">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="text-xl font-black text-blue-950">
                        محرك مزامنة بوابة ناجز المباشر
                      </h3>
                      <p className="text-xs text-blue-900 font-bold mt-1">
                        يُرجى تشغيل المحرك لبدء الربط مع بوابة وزارة العدل.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 border border-slate-100 p-5 rounded-2xl">
                    <p className="text-[10px] text-slate-700 font-black mb-3 leading-relaxed">
                      يستخدم هذا الموديول "بوابة النفاذ الوطني" (IAM) بشكل آلي
                      لسحب ملفات القضايا والقرارات لعام{" "}
                      {new Date().getFullYear()}. تظهر النتائج في لوحة التحكم
                      الرئيسية فور انتهاء السجل.
                    </p>
                    <button
                  ...
                  <div className="bg-white rounded-2xl p-6 border border-slate-200 space-y-4">
                    ...
                    <div className="h-44 overflow-y-auto space-y-2 border border-slate-100 p-4 rounded-xl font-mono text-[11px] text-emerald-600 bg-slate-50 custom-scrollbar">
                      {najizSyncLog.length === 0 ? (
                        <span className="text-slate-200 font-bold font-bold block text-center py-12">
                          مستعد لبدء الربط التلقائي...
                        </span>
                      ) : (
                        najizSyncLog.map((log, i) => (
                          ...
                          >
                            <span className="text-right leading-normal font-bold">
                              {log.msg}
                            </span>
                            <span className="text-slate-200 font-bold shrink-0 select-none">
                              [{log.time}]
                            </span>
                          </div>
                        ))
              ...
              <div className="flex items-center gap-4 border-b border-slate-100 pb-4">
                ...
                <div className="text-right">
                  <h3 className="text-xl font-black text-slate-900">
                    مساعد المحاماة الذكي بـ Gemini
                  </h3>
                  <p className="text-xs text-slate-700 font-bold mt-1">
                    مستشارك السحابي لصياغة مذكرات الدفاع، العقود والمذكرات
                    الجوابية، والبحث في الأنظمة القضائية واللوائح التنفيذية.
                  </p>
                </div>
              </div>
                    ...
                    >
                      ...
                      <div>
                        <div className="flex justify-between items-start mb-6">
                          <span className="text-[10px] font-black text-slate-200 font-bold font-mono tracking-wide">
                            {c.caseNumber}
                          </span>
                          <span
                        ...
                        <h4 className="font-black text-lg text-white mb-4 leading-tight">
                          {c.caseName}
                        </h4>
                        <div className="space-y-3 text-[11px] text-slate-300 font-bold mb-6 pr-1">
                          <p className="flex items-center gap-2.5">
                            <Users className="w-4 h-4 text-blue-600" />{" "}
                            الموكل/العميل: {c.clientName}
                          </p>
                          <p className="flex items-center gap-2.5">
                            <Globe className="w-4 h-4 text-emerald-600" />{" "}
                            المحكمة: {c.courtName || "المحكمة العامة"}
                          </p>
                          {c.nextSessionDate && (
                            <p className="flex items-center gap-2.5">
                      ...
                      <div className="pt-5 border-t border-slate-100">
                        <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100">
                          <p className="text-[10px] text-slate-200 font-bold font-black mb-1">
                            ملخص الخصم:
                          </p>
                          <p className="text-xs font-black text-white">
                            {c.opponentName || "غير مسجل"}
                          </p>
                        </div>
                      </div>
                    </motion.div>
                ...
                {tasksLoading ? (
                  ...
                  <div className="text-center py-8 border border-dashed border-slate-700 rounded-2xl">
                    ...
                    <p className="text-slate-500 text-sm">
                      لا توجد مهام مُسندة إليك حالياً
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                ...
                {myClients.length === 0 ? (
                  ...
                  myClients.map((client) => (
                    ...
                    >
                      ...
                      <div>
                        <h4 className="font-black text-lg text-slate-900 mb-2 leading-tight">
                          {client.name}
                        </h4>
                        <p className="text-[10px] text-slate-200 font-bold font-black mb-6">
                        ...
                        <div className="space-y-3 text-[11px] text-slate-700 font-bold mb-6 pr-1">
                          ...
                          <p className="flex items-center gap-2.5">
                            <Briefcase className="w-4 h-4 text-amber-500" /> نوع
                            العميل: {client.isCompany ? "شركة / مؤسسة" : "فرد"}
                          </p>
                        </div>
                      </div>
            ...
            <div className="max-w-5xl mx-auto space-y-10">
              <div className="text-center space-y-4">
                <h3 className="text-3xl font-black text-slate-900">
                  مرصد الأنظمة والبحث القضائي الذكي 🔍
                </h3>
                <p className="text-slate-700 font-bold max-w-2xl mx-auto">
                  ابحث في كافة الأنظمة واللوائح والتعاميم الصادرة من وزارة العدل
                  والمحاكم السعودية باستخدام تقنيات الذكاء الاصطناعي.
                </p>
              </div>
              ...
              <div className="bg-white p-8 rounded-[3rem] border border-blue-100 shadow-xl shadow-blue-600/5 space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    <button
                      key={sys}
                      className="px-6 py-4 bg-slate-50 hover:bg-blue-50 text-slate-200 font-bold hover:text-blue-600 rounded-2xl text-xs font-black border border-slate-100 hover:border-blue-200 transition-all"
                    >
                      {sys}
                    </button>
                  ))}
          ...
          {activePortalTab === "documents" && (
            <div className="bg-white border border-slate-200 p-10 rounded-[2.5rem] shadow-sm space-y-8">
              ...
              <p className="text-xs text-slate-700 font-bold leading-relaxed">
                قم بتحميل الأقراص الصكوك وتفويض التوكيل المرتبط بـ{" "}
                {loggedInEmployee.name}. الملفات ترفع تلقائياً على خزانة الحساب
              ...
              <div className="dropzone border-2 border-dashed border-slate-200 bg-slate-50 hover:bg-white rounded-3xl p-12 text-center cursor-pointer transition-all hover:border-blue-400 flex flex-col items-center gap-4">
                ...
                <span className="text-[11px] text-slate-200 font-bold font-bold">
                  صيغ مدعومة: PDF, DOC, PNG, JPEG حتى حجم 30 ميغابايت
                </span>
              </div>
            </div>
          )}
            ...
            >
              <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                ...
                <button
                  ...
                  className="text-slate-200 font-bold hover:text-rose-600 transition-all"
                >
              ...
              <form onSubmit={handleApplyLeave} className="p-8 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-700 uppercase tracking-widest block pr-2">
                      نوع الإجازة
                    </label>
                    <select
                      name="type"
                ...
                <div className="pt-4 flex gap-4">
                  <button
                    type="submit"
                    className="flex-1 bg-blue-900 hover:bg-blue-800 text-white py-4 rounded-2xl font-black text-xs shadow-lg transition-all active:scale-95"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setShowLeaveForm(false)}
                    className="px-8 bg-slate-100 hover:bg-slate-200 text-slate-200 font-bold py-4 rounded-2xl font-black text-xs transition-all"
                  >
                    إلغاء
                  </button>
                </div>
              ...
              {geoLocating ? (
                <>
                  ...
                  <p className="text-sm font-bold text-slate-700 mb-8 max-w-[250px]">
                    يرجى الانتظار بينما نقوم بالتحقق من إحداثياتك للمطابقة مع
                    النطاق المسموح.
                  </p>
                  <button
                    onClick={() => {
                    ...
                    className="text-slate-200 font-bold font-bold hover:text-slate-200 font-bold"
                  >
                    إلغاء العملية
                  </button>
                </>
                ...
                <>
                  ...
                  <button
                    ...
                    className="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 py-4 rounded-2xl font-black text-xs transition-all"
                  >
                    إغلاق
                  </button>
                </>
