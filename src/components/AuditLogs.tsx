      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div className="bg-[#0a1628] border border-[#1e3a5f]/80 p-6 rounded-2xl relative overflow-hidden shadow-xs hover:border-[#1e3a5f] transition-all flex flex-col justify-between">
          <div className="flex justify-between items-start mb-4">
            <div className="space-y-1">
              ...
              <h4 className={`text-xs ${TEXT_COLORS.onLightMuted} font-black`}>API Transactions Logs</h4>
            </div>
            <div className={`p-2.5 bg-slate-50 border border-slate-100 ${getDynamicTextColor('bg-slate-50')} rounded-xl`}>
              <Terminal className={`w-5 h-5 ${TEXT_COLORS.onLightMuted}`} />
            </div>
          </div>
          <div className={`text-3xl font-black ${getDynamicTextColor('bg-slate-50')} font-mono tracking-tight`}>{logs.length}</div>
          <p className={`text-[10px] ${getDynamicTextColor('bg-slate-50')} font-bold mt-2 pt-2 border-t border-slate-100`}>كامل المدخلات ومحاولات التحصيل المرصودة</p>
        </div>
        ...
        <div className="bg-[#0a1628] border border-[#1e3a5f]/80 p-6 rounded-2xl relative overflow-hidden shadow-xs hover:border-[#1e3a5f] transition-all flex flex-col justify-between">
          <div className="flex justify-between items-start mb-4">
            <div className="space-y-1">
              ...
              <h4 className={`text-xs ${TEXT_COLORS.onLightMuted} font-black`}>Control & Modifications</h4>
            </div>
          ...
          <p className={`text-[10px] ${getDynamicTextColor('bg-slate-50')} font-bold mt-2 pt-2 border-t border-slate-100 font-sans`}>عمليات الإضافة، تعديل أو حذف السجلات</p>
        </div>
        ...
        <div className="bg-[#0a1628] border border-[#1e3a5f]/80 p-6 rounded-2xl relative overflow-hidden shadow-xs hover:border-[#1e3a5f] transition-all flex flex-col justify-between">
          <div className="flex justify-between items-start mb-4">
            <div className="space-y-1">
              ...
              <h4 className={`text-xs ${TEXT_COLORS.onLightMuted} font-black`}>Security Authenticity</h4>
            </div>
          ...
          <p className={`text-[10px] ${getDynamicTextColor('bg-slate-50')} font-bold mt-2 pt-2 border-t border-slate-100`}>برشام تشفير الجلسات والإخطار التلقائي</p>
        </div>
        ...
        <div className="bg-[#0a1628] border border-[#1e3a5f]/80 p-6 rounded-2xl relative overflow-hidden shadow-xs hover:border-blue-300 transition-all flex flex-col justify-between">
          <div>
            <div className="flex justify-between items-start mb-4">
              <div className="space-y-1">
                <span className={`text-[10px] ${getDynamicTextColor('bg-slate-50')} font-black uppercase tracking-wider block`}>نسخ احتياطي فوري</span>
                <h4 className={`text-xs ${TEXT_COLORS.onLightMuted} font-black`}>Encrypted Backup Cloud</h4>
              </div>
              <div className="p-2.5 bg-purple-50 border border-purple-100 text-purple-900 rounded-xl">
                <Database className="w-5 h-5 text-purple-600" />
              </div>
            </div>
            <div className={`text-xl font-black ${getDynamicTextColor('bg-slate-50')} font-mono`}>
              {backups.length > 0 ? backups[0].databaseSize : 'قيد الانتظار'}
            </div>
            <p className={`text-[10px] ${getDynamicTextColor('bg-slate-50')} font-bold truncate mt-1`}>آخر تجميد سحابي: {backups.length > 0 ? new Date(backups[0].timestamp).toLocaleDateString('ar-SA') : 'معلق'}</p>
          </div>
      ...
      <div className="bg-[#0a1628] border border-[#1e3a5f]/80 p-8 rounded-[2rem] shadow-xs space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-100 pb-5 gap-4">
          <div className="flex items-center gap-4.5">
            ...
            <div>
              <h2 className={`text-lg font-black ${getDynamicTextColor('bg-slate-50')}`}>سجل عمليات جدولة النسخ الاحتياطي السحابي</h2>
              <p className={`text-xs ${getDynamicTextColor('bg-slate-50')} font-bold mt-0.5`}>تفاصيل ومطابقة الكتل السحابية المؤرشفة تلقائياً (Automated backups)</p>
            </div>
          </div>
        ...
        {backups.length === 0 ? (
          <div className={`text-center py-10 text-xs font-bold ${getDynamicTextColor('bg-slate-900')} font-bold bg-slate-50 rounded-2xl border border-slate-100`}>
            ...
            {backups.slice(0, 3).map((bk, idx) => (
              <div key={bk.id || idx} className="bg-slate-50 border border-[#1e3a5f] p-5 rounded-2xl space-y-3.5 hover:border-[#1e3a5f] transition-all flex flex-col justify-between">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`text-[10px] ${getDynamicTextColor('bg-slate-50')} font-mono font-black flex items-center gap-1`}>
                    <Clock className={`w-3 h-3 ${getDynamicTextColor('bg-slate-900')} font-bold`} />
                    {new Date(bk.timestamp).toLocaleTimeString('ar-SA', { hour: '2-digit', minute: '2-digit' })}
                  </span>
                </div>
                <div>
                  <span className={`text-[11px] ${getDynamicTextColor('bg-slate-900')} font-bold block font-bold uppercase tracking-wider mb-0.5`}>رمز الكتلة السحابية الآمنة</span>
                  <strong className={`${getDynamicTextColor('bg-slate-50')} font-mono text-xs font-black block truncate`}>{bk.id}</strong>
                </div>
                <div className="grid grid-cols-2 gap-4 text-xs pt-3 border-t border-slate-100">
                  <div>
                    <span className={`text-[11px] ${getDynamicTextColor('bg-slate-900')} font-bold block font-bold uppercase tracking-wider mb-0.5`}>حجم البيانات</span>
                    <strong className={`${getDynamicTextColor('bg-slate-50')} font-black`}>{bk.databaseSize}</strong>
                  </div>
                  <div>
                    <span className={`text-[11px] ${getDynamicTextColor('bg-slate-900')} font-bold block font-bold uppercase tracking-wider mb-0.5`}>جداول مفحوصة</span>
                    <strong className={`${getDynamicTextColor('bg-slate-50')} font-black`}>{bk.tablesCount} جدول</strong>
                  </div>
                </div>
                <div className={`text-[10px] ${getDynamicTextColor('bg-slate-900')} font-bold pt-3 flex justify-between items-center bg-slate-200/40 p-3 rounded-xl`}>
                  <span>المستودع: <span className={`font-extrabold ${getDynamicTextColor('bg-slate-50')}`}>{bk.destination}</span></span>
                  <span>المنفذ: <span className={`font-extrabold ${getDynamicTextColor('bg-slate-50')}`}>{bk.triggeredBy}</span></span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="bg-[#0a1628] border border-[#1e3a5f]/80 rounded-[2rem] shadow-sm overflow-hidden">
        ...
        <div className="p-6 md:p-8 border-b border-slate-100 bg-slate-50/50 flex flex-col md:flex-row items-stretch md:items-center justify-between gap-6">
          <div className="flex flex-wrap items-center gap-6">
            <div className="flex items-center gap-3">
              <span className={`text-xs ${getDynamicTextColor('bg-slate-50')} font-black uppercase tracking-wider`}>المستخدم المنفذ:</span>
              <select
                value={selectedUser}
            ...
            <div className="flex items-center gap-3">
              <span className={`text-xs ${getDynamicTextColor('bg-slate-50')} font-black uppercase tracking-wider`}>نوع العملية:</span>
              <select
        ...
        {loading ? (
          <div className="p-20 text-center space-y-4">
            <RefreshCw className={`w-10 h-10 ${getDynamicTextColor('bg-slate-50')} animate-spin mx-auto`} />
            <div className={`text-xs ${getDynamicTextColor('bg-slate-50')} font-black tracking-widest uppercase`}>جاري استيراد وتحديث منصة الحوكمة...</div>
          </div>
        ) : filteredLogs.length === 0 ? (
          <div className="p-20 text-center space-y-5">
            <div className={`w-16 h-16 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-center mx-auto ${getDynamicTextColor('bg-slate-900')} font-bold`}>
              <ShieldAlert className={`w-8 h-8 ${getDynamicTextColor('bg-slate-900')} font-bold`} />
            </div>
            <div className={`text-xs font-black ${getDynamicTextColor('bg-slate-50')}`}>لا توجد أي سجلات أمنية مطابقة لمعايير التصفية الحالية.</div>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-right border-collapse">
              ...
              <tbody className={`divide-y divide-slate-100 ${getDynamicTextColor('bg-slate-50')} font-bold text-xs md:text-sm`}>
                {filteredLogs.map((log) => {
                  const isExpanded = expandedLogId === log.id;
                  return (
                    <React.Fragment key={log.id}>
                      <tr className={`hover:bg-slate-50/50 transition-colors ${getDynamicTextColor('bg-slate-50')}`}>
                        <td className={`py-4 px-6 ${getDynamicTextColor('bg-slate-50')} font-extrabold`}>{log.user === 'admin' ? 'المدير العام 👑' : log.user}</td>
                        ...
                        <td className={`py-4 px-6 ${getDynamicTextColor('bg-slate-50')} text-xs font-mono whitespace-nowrap`}>
                          {new Date(log.timestamp).toLocaleTimeString('ar-SA')}
                        </td>
                        <td className={`py-4 px-6 font-mono ${getDynamicTextColor('bg-slate-50')} text-xs truncate max-w-[240px]`}>
                          {log.path}
                        </td>
                        <td className="py-4 px-6 text-left">
                          {log.request_payload ? (
                            ...
                            <span className={`${getDynamicTextColor('bg-slate-900')} font-bold text-[10px] uppercase tracking-widest pl-4`}>بدون محتوى</span>
                          )}
                        </td>
                      </tr>
                               ...
                               <div className="p-6 md:p-8 border-t border-b border-[#1e3a5f]/60">
                                  <div className="bg-slate-950 border border-slate-800 rounded-2xl p-6 shadow-md relative overflow-hidden">
                                     ...
                                     <div className="relative z-10 flex justify-between items-center mb-4">
                                       <span className={`text-[10px] font-mono font-bold ${getDynamicTextColor('bg-slate-50')} tracking-wider flex items-center gap-1`}>
