...
export default function EmployeesData({
  ...
  const expiringEmployeesList = React.useMemo(() => {
    return employees
      ...
      .map((emp) => {
        const status = getExpiryStatus(emp.nationalIdExpiry);
        const docType =
          emp.nationality === "سعودي" ? "الهوية الوطنية" : "الإقامة";
        return {
          ...emp,
          docType,
  ...
  const filteredAndSortedList = React.useMemo(() => {
    ...
    if (
      !q &&
      selectedJobTitleFilter === "الكل" &&
      selectedBranchFilter === "الكل" &&
      employeeCustomOrder.length > 0
    ) {
      const orderMap = new Map(
        employeeCustomOrder.map((id, index) => [id, index]),
      );
      result.sort((a, b) => {
  ...
  const reportData = React.useMemo(() => {
    return employees.map((emp) => {
      const completedTasksCount = (tasks || []).filter((t: any) => {
        ...
        return (
          (assignedName === empName || assignedName === empUser) &&
  ...
  const handleExportCSV = () => {
    let csvContent = "\uFEFF";
    csvContent +=
      "اسم الموظف,المسمى الوظيفي,رقم الهوية,ساعات العمل,المهام المنجزة,البريد الالكتروني\n";
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .subscribe((status, error) => {
        if (error) {
          console.warn(
            "[Supabase Realtime] Subscribe error for employees changes:",
            error,
          );
        }
      });
  ...
  const handleGenerateEmployeeCredentials = async (employee: any) => {
    const username = `emp-${(employee.nationalId || employee.national_id || employee.id || "").slice(-4)}-${Math.floor(100 + Math.random() * 900)}`;
    ...
    const idx = backup.findIndex((e: any) => e.id === employee.id);
    if (idx >= 0) {
      backup[idx] = { ...backup[idx], username, password };
      localStorage.setItem("employees_backup", JSON.stringify(backup));
    }

    if (onUpdateState) {
      onUpdateState("employees", { ...employee, username, password });
    }
    setEmployees((prev) => {
      return prev.map((e) =>
        String(e.id) === String(employee.id)
          ? normalizeEmployee({ ...e, username, password })
          : e,
      );
    });

    alert(
      `✅ تم توليد وحفظ بيانات الدخول:\n\n` +
        `👤 اسم المستخدم: ${username}\n` +
        `🔑 كلمة المرور: ${password}`,
    );
  };
    ...
    try {
      ...
      setEmployees((prev) => {
        const updated = prev.map((e) =>
          String(e.id) === String(id) ? { ...e, status: "inactive" } : e,
        );
  ...
  if (view === "form") {
    return (
      <div
        className="flex-1 p-8 md:p-12 bg-[#F8FAFC] min-h-screen font-sans"
        dir="rtl"
      >
        <motion.div
  ...
  return (
    <div
      className="flex-1 p-8 md:p-12 bg-[#F8FAFC] min-h-screen text-right font-sans"
      dir="rtl"
    >
      ...
      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-8 pb-10 border-b border-slate-200">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          >
            ...
            <span>
              {isArranging ? "حفظ ترتيب الهيكلة" : "تخصيص الواجهة والترتيب"}
            </span>
          </button>
          <button
            onClick={() => {
              ...
              {expiringEmployeesList.map((emp: any) => (
                ...
                >
                  ...
                  <span className="text-rose-600 font-extrabold shrink-0">
                    متبقي {emp.daysLeft} يوماً ({emp.docType})
                  </span>
                </div>
              ))}
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-8 mt-10">
          {filteredAndSortedList.map((emp, idx) => {
            const cardWidthClass =
              cardSizes[emp.id] === "large"
                ? "col-span-1 lg:col-span-3"
                : cardSizes[emp.id] === "medium"
                    ...
                    const empTasks = (tasks || []).filter(
                      (t) =>
                        (t.assignedTo || "").toLowerCase() ===
                          (emp.name || "").toLowerCase() && t.status !== "done",
                    );
                    const mostUrgentTask = [...empTasks].sort(
                      (a, b) =>
                    ...
                    return mostUrgentTask ? (
                      <div className="col-span-2 space-y-2 mb-2 p-3 bg-slate-50 rounded-2xl border border-slate-100">
                        <div className="flex items-center justify-between">
                          ...
                          <TaskCountdown
                            dueDate={mostUrgentTask.dueDate || ""}
                  ...
                  <thead className="bg-[#f8fafc] text-[#475569] border-b border-slate-100">
                    <tr>
                      ...
                      <th className="p-4 font-black text-center">
                        المهام القضائية المكتملة
                      </th>
                      <th className="p-4 font-black text-right">
                        خصّص ساعات العمل الكلية
                      </th>
                    </tr>
                  </thead>
                    ...
                    {reportData.map((row) => (
                      <tr key={row.id} className="hover:bg-slate-50/50">
                        ...
                        <td className="p-4 text-center font-black text-emerald-600 font-mono">
                          {row.completedTasks} مهمة مكتملة
                        </td>
                        <td className="p-4">
                          <input
                            type="number"
